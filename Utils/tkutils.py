"""
This file contains all the helper functions use to build UI using tkinter
"""
import shutil
import os
import threading
...
def show_result_frame(root_window, *args, **kwargs):
    ...
    def try_another():
        kwargs["url_entry"].delete(0, END)
        result_frame.destroy()
        kwargs["search_button"]["state"] = NORMAL

    def download_video():
        if resolution_dropdown.get():
            # Disabling the essential buttons to prevent running multiple threads
            download_button["state"] = DISABLED
            kwargs["search_button"]["state"] = DISABLED
            ...
            try:
                video_progressBar.fileSize = pytube.YouTube(url=kwargs["video_url"])\
                    .streams.get_by_itag(resolution_options[f"{resolution_dropdown.get()}"]).filesize
                streamQuery = pytube.YouTube(url=kwargs["video_url"],
                ...
                if streamQuery.audio_codec and streamQuery.audio_codec == "mp4a.40.2":
                    ...
                    pytube.YouTube(url=kwargs["video_url"],
                                             on_progress_callback=video_progressBar.progress_Check)\
                        .streams.get_by_itag(140).download(filename="Audio_0mpOlokeLYrCsQ64hk5K", output_path="cached/")
                ...
                download_file = pytube.YouTube(url=kwargs["video_url"],
                                               on_progress_callback=video_progressBar.progress_Check)\
                    .streams.get_highest_resolution()
                download_file.download(output_path=f"{r_file_path if 'r_file_path' in globals() else helperFunctions.file_path()}")
                # Enabling the search button for next search
                kwargs["search_button"]["state"] = NORMAL
                download_button["state"] = NORMAL
                another_url_button["state"] = NORMAL

    thumbnail = helperFunctions.show_image(image_url=kwargs["thumbnail_url"])
    ...
    title = Label(title_frame, text=kwargs["title"], font=("bold", 12), bg="#2b2929", fg="white", wraplength=500)
    ...
    resolution_dropdown = ttk.Combobox(result_frame, textvariable=resolution_dropdown_value, width=33, font=('bold', 14))
